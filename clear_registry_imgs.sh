#!/bin/bash
#curl -u "admin:Harbor12345" -X GET -H "Content-Type: application/json" "http://127.0.0.1/api/projects" > projects.json
#cat projects.json | jq .[].name
#cat projects.json | jq '.[] | select(.name == "testttt")| .project_id'
#curl -u "admin:Harbor12345" -X GET -H "Content-Type: application/json" "http://127.0.0.1/api/repositories?project_id=12" > repo.json
#curl -ksS -u "admin:Harbor12345" -X GET -H "Content-Type: application/json" "http://127.0.0.1/api/repositories/testttt/nginx/tags" | jq .[].name -r | sort -rn| awk 'NR>3 {print}'
#curl -ksS -u "admin:Wise2c2017_crm" -X DELETE -w %{http_code} -H "Content-Type: application/json" "http://127.0.0.1/api/repositories/testttt/nginx/tags/v2"

harbor_endpoint=127.0.0.1
harbor_username=admin
harbor_password=password
harbor_schema=http
KEEP_TAGs_NUM=3

tag=`curl -ksS -u "admin:Harbor12345" -X GET -H "Content-Type: application/json" "http://127.0.0.1/api/repositories/testttt/nginx/tags" | jq .[].name -r | sort -rn| awk 'NR>3 {print}'`
#echo ${tag}
#if [ -z ${tag} ]
#then 
#    echo null
...
#echo "curl -ksS -u "admin:Wise2c2017_crm" -X DELETE -w %{http_code} -H "Content-Type: application/json" "http://127.0.0.1/api/repositories/testttt/nginx/tags/${tag}""
#curl -ksS -u "admin:Wise2c2017_crm" -X DELETE -w %{http_code} -H "Content-Type: application/json" "http://127.0.0.1/api/repositories/testttt/nginx/tags/${tag}"
